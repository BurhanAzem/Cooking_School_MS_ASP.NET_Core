...
namespace Cooking_School_ASP.NET.Services
{
    public class ChefService : IChefService
    {
        ...
        public async Task<ResponsDto<ChefDTO>> UpdateUser(int id, UpdateChefDto UpdatechefDto)
        {
            ...
            if (UpdatechefDto.BirthDate != null) { chef.BirthDate = UpdatechefDto.BirthDate; }
            ...
            if (UpdatechefDto.PhoneNumber != 0) { chef.PhoneNumber = (int)UpdatechefDto.PhoneNumber; }
            if (UpdatechefDto.Cv is not null) 
            {
                ...
                fileName = Path.GetFileName(fileName);
                // Delete exiting file
                System.IO.File.Delete(Path.Combine(Directory.GetCurrentDirectory(), "wwwroot\\cv", fileName));
                // Save new file
                //string fileName = Guid.NewGuid() + Path.GetFileName(UploadImage.FileName);
                string uploadpath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot\\cv", fileName);
                var stream = new FileStream(uploadpath, FileMode.Create);

                await UpdatechefDto.Cv.CopyToAsync(stream);
